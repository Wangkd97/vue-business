                  ...
                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a href="javascript:void 0" class="item-check-btn"
                         :class="{'check':item.productChecked}"
                         @click="changeCheck(item)">
                        <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                      </a>
                    </div>
                    <div class="cart-item-pic">
                      <img :src="'http://img.cdn.imbession.top/'+item.productMainImage" alt="图片">
                    </div>
                  ...
                  <div class="cart-tab-3">
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="quantity">
                          <a href="javascript:void 0" @click="minNum(item)">-</a>
                          <input style="width: 70px" type="text" v-model="item.quantity" >
                          <a href="javascript:void 0" @click="addNum(item)">+</a>
                        </div>
                      </div>
                      <div class="item-stock" v-if="item.quantity<item.productStock">有货</div>
                      <div class="item-stock" v-if="item.quantity>item.productStock">超出存货量</div>
                    </div>
                  </div>
                  <div class="cart-tab-4">
                    <div class="item-price-total">{{item.quantity*item.productPrice|formatMoney}}</div>
                  </div>
              ...
              <div class="item-all-check">
                <a href="javascript:void 0">
                    <span class="item-check-btn"
                          :class="{'check':isallchecked}">
                      <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                    </span>
                  <span @click="changeAllCheck(true)">全选</span>
                </a>
              </div>
              <div class="item-all-del">
                <a href="javascript:void 0" class="item-del-btn">
                  <span @click="changeAllCheck(false)">/取消全选</span>
                </a>
              </div>
            </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...mapActions(['setUserInfo','setdingdanNo','setAddress','isShowFooterBar','setaddressId']),
      ...mapGetters(['getJudgeAddress','getAddress','getdingdanNo',]),
      getAddress:function(){
        var _vm=this
        this.service.get("/shipping/lookShippingMessageList")
          .then(function (response) {

          console.log("response=========="+response)
          _vm.addressList = response.data.data.shippings
            console.log( "===addresslist===="+ _vm.addressList)
            console.log( _vm.addressList)
          _vm.firstAddress=_vm.addressList[0]
            _vm.setaddressId(_vm.firstAddress.id)

            console.log("===firstaddress===="+_vm.firstAddress)
            console.log( _vm.firstAddress)
            //_vm.setAddress()
         // console.log(_vm.getAddress()+"=========quanju--dizhi------")
          //  console.log(_vm.getAddress())

            //console.log(_vm.getAddress().receiverName+"=========quanju--dizhi------")
          console.log(_vm.addressList)
        })
      ...
      getCarts:function () {
        ...
        this.service.get("/cart/searchCartProductList").then(function (response) {
          ...
          _vm.productList=response.data.data.cartproductViewObjectList
          _vm.totalMoney=response.data.data.cartTotalPrice
          _vm.isallchecked=response.data.data.allchecked
        }).catch(function (error) {
          console.log(error)
        })
      },
      addNum:function (item) {
        var _vm=this
        //通过axios发请求  post
        this.service.post("/cart/alterNum",{
